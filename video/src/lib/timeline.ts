...
export type ScreenScript = { index: number; title: string; cues: Cue[] };
...
export type TScreen = { index: number; from: number; frames: number; cues: TCue[] };
...
export function buildTimeline(script: Script, manifest: Manifest): Timeline {
  ...
  for (const sc of script.screens) {
    ...
    for (const c of sc.cues) {
      ...
      const tc: TCue = { ...c, from: t, frames: audio + extra + GAP, screen: sc.index, src: m?.src ?? "" };
      list.push(tc); cues.push(tc);
      t += tc.frames;
    }
    t += 10;
    screens.push({ index: sc.index, from, frames: t - from, cues: list });
  }
